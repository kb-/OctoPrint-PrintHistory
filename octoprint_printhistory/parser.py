...
class Slic3rParser(BaseParser):
    """covers parsing of gcode from OSS Slic3r as well as PrusaSlicer, which is 
    derived from it"""
    def __init__(self):
        self.name = "slic3r_or_prusaslicer"
        self.version = None
    ...
    def detect(self, gcode_file):
        ...
        line = gcode_file.readline()
        gcode_file.seek(0)
        if re.search(r"Slic3r", line) or re.search(r"PrusaSlicer", line):
...
class TestUniversalParser(unittest.TestCase):
    def setUp(self):
        ...
        self.prusaslicer_file = "parser_test/prusaslicer_test.gcode"
    ...
    def test_slic3r_detection(self):
        ...
        self.assertEqual(uparser.name, "slic3r_or_prusaslicer")
    ...
    def test_prusaslicer_detection(self):
        uparser = UniversalParser(self.prusaslicer_file)
        self.assertEqual(uparser.name, "slic3r_or_prusaslicer")

    def test_prusaslicer_parse(self):
        uparser = UniversalParser(self.prusaslicer_file)
        result = uparser.parse()
        self.assertEqual(len(result), 233)
        self.assertIn("thin_walls", result)
        self.assertIn("support_material_extrusion_width", result)
        self.assertEqual(result["bridge_speed"], "20")
        self.assertEqual(result["slicer_version"], "2.1.0")
